...
func (aa *AudioAnalyzer) InferCategoryWithConfidence(meta *AudioMetadata, filename string) CategoryResult {
	...
	// filename pattern matching (high confidence)
	if strings.Contains(filenameLower, "scream") || strings.Contains(filenameLower, "grunt") ||
		strings.Contains(filenameLower, "groan") || strings.Contains(filenameLower, "speech") ||
		strings.Contains(filenameLower, "dialogue") || strings.Contains(filenameLower, "voice") {
		scores["SFX_Voice"] += 0.8
	}
	if strings.Contains(filenameLower, "creature") || strings.Contains(filenameLower, "monster") ||
		strings.Contains(filenameLower, "animal") || strings.Contains(filenameLower, "beast") ||
		strings.Contains(filenameLower, "roar") || strings.Contains(filenameLower, "growl") ||
	...
	if strings.Contains(filenameLower, "explosion") || strings.Contains(filenameLower, "explode") ||
		...
		strings.Contains(filenameLower, "slam") || strings.Contains(filenameLower, "thud") ||
		strings.Contains(filenameLower, "bang") || strings.Contains(filenameLower, "boom") {
		scores["SFX_Impact"] += 0.8
	}
	if strings.Contains(filenameLower, "footstep") || strings.Contains(filenameLower, "walk") ||
		strings.Contains(filenameLower, "run") || strings.Contains(filenameLower, "jump") ||
	...
	if strings.Contains(filenameLower, "machine") || strings.Contains(filenameLower, "gear") ||
		...
		scores["SFX_Mechanical"] += 0.8
	}
	if strings.Contains(filenameLower, "door") || strings.Contains(filenameLower, "creak") ||
		strings.Contains(filenameLower, "object") || strings.Contains(filenameLower, "item") ||
		strings.Contains(filenameLower, "pickup") || strings.Contains(filenameLower, "drop") {
		scores["SFX_Object"] += 0.8
	}
	if strings.Contains(filenameLower, "button") || strings.Contains(filenameLower, "click") ||
		strings.Contains(filenameLower, "select") || strings.Contains(filenameLower, "hover") ||
	...
	if strings.Contains(filenameLower, "music") || strings.Contains(filenameLower, "song") ||
		strings.Contains(filenameLower, "track") || strings.Contains(filenameLower, "score") {
		scores["Music"] += 0.8
	}
	if strings.Contains(filenameLower, "siren") || strings.Contains(filenameLower, "alarm") {
		scores["SFX_Alarm"] += 0.8
	}

	// metadata-based scoring (medium confidence)
	if meta.Duration > 0 {
		if meta.Duration < 2*time.Second {
			scores["SFX_UI"] += 0.6
		} else if meta.Duration < 5*time.Second {
